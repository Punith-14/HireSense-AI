...
class InterviewEngine:
    ...
    def start(self, role, mode="technical", user_email=None, full_name=None):
        session, interview = self.memory.start_session(role=role, mode=mode, user_email=user_email, full_name=full_name)
    ...
    def answer(self, session_id, answer_text, speech_metrics=None, vision_metrics=None):
        ...
        context = {
            ...
            "difficulty": current_question.get("difficulty", session.difficulty),
            "answer_text": answer_text,
            "speech_metrics": speech_metrics or {},
            "vision_metrics": vision_metrics or {},
            "skills": prompt_engine.skills_for_role(session.role),
        }
    ...
    def _generate_question(self, session, interview):
        context = self.memory.build_context(session, interview)
        context["skills"] = prompt_engine.skills_for_role(session.role)
        prompt = prompt_engine.question_prompt(context)
        return self._normalize_question(
            self.llm.generate_json(prompt, fallback=lambda: self._fallback_question(context))
        )
    ...
    def _generate_follow_up_or_next(self, session, interview, answer_text, current_question, evaluation):
        context = {
            ...
            "answer_text": answer_text,
            "evaluation": evaluation,
            "next_difficulty": evaluation.get("next_difficulty", session.difficulty),
        }
        if evaluation.get("follow_up_recommended"):
            prompt = prompt_engine.follow_up_prompt(context)
            return self._normalize_question(
                self.llm.generate_json(prompt, fallback=lambda: self._fallback_follow_up(context))
            )
